...
class Auth:
    def __init__(self, app, db, user_model=None, group_model=None, membership_model=None, permission_model=None):
        ...
        self.ext.use_database(db, user_model, group_model, membership_model, permission_model)
        self.ext.init_forms()
    ...
    def has_membership(
        ...
        if group and user:
            if (
                self.models["membership"]
                .where(
                    lambda m: (m.table[self.ext.relation_names["user"]] == user)
    ...
    def has_permission(
        ...
        if not parent:
            return False
        return parent[self.ext.relation_names["permission"] + "s"].where(query).count() > 0

    #: operations
    def create_group(self, role: str, description: str = "") -> _RecordReference:
        res = self.models["group"].create(role=role, description=description)
        return res.id
    ...
    def add_membership(self, group: Union[str, int, Row], user: Optional[Row] = None) -> _RecordReference:
        ...
        if not user and self.user:
            user = self.user.id
        res = getattr(group, self.ext.relation_names["user"] + "s").add(user)
        return res.id

    def remove_membership(self, group: Union[str, int, Row], user: Optional[Row] = None):
        if isinstance(group, int):
            group = self.models["group"].get(group)
        elif isinstance(group, str):
            group = self.group_for_role(group)
        if not user and self.user:
            user = self.user.id
        return getattr(group, self.ext.relation_names["user"] + "s").remove(user)
    ...
    def change_user_status(self, user: Union[int, Row], status: str) -> int:
        return self.ext.db(self.models["user"].id == user).update(registration_key=status)
    ...
    #: emails decorators
    def registration_mail(self, f: Callable[[Row, Dict[str, Any]], bool]) -> Callable[[Row, Dict[str, Any]], bool]:
        self.ext.mails["registration"] = f
        return f

    def reset_password_mail(self, f: Callable[[Row, Dict[str, Any]], bool]) -> Callable[[Row, Dict[str, Any]], bool]:
...
class AuthPipe(Pipe):
    ...
    def session_open(self):
        ...
        if authsess.last_visit + timedelta(seconds=authsess.expiration) < visit_dt:
            del session.auth
        #: does session need re-sync with db?
            ...
            if (visit_dt - authsess.last_visit).seconds > min(authsess.expiration / 10, 600):
