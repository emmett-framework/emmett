...
def test_all_recipients(mailer):
    msg = mailer.mail(
        subject="subject", recipients=["somebody@here.com"], cc=["cc@example.com"], bcc=["bcc@example.com"]
    )
...
def test_sender(mailer):
    msg = mailer.mail(subject="subject", sender="ÄÜÖ → ✓ <from@example.com>>", recipients=["to@example.com"])
    assert "From: =?utf-8?b?w4TDnMOWIOKGkiDinJM=?= <from@example.com>" in str(msg)


def test_sender_as_tuple(mailer):
    msg = mailer.mail(subject="testing", sender=("tester", "tester@example.com"))
...
def test_bad_subject(mailer):
    msg = mailer.mail(subject="testing\r\n", sender="from@example.com", body="testing", recipients=["to@example.com"])
    with pytest.raises(RuntimeError):
        msg.send()


def test_subject(mailer):
    msg = mailer.mail(subject="sübject", sender="from@example.com", recipients=["to@example.com"])
...
def test_bad_sender(mailer):
    msg = mailer.mail(subject="testing", sender="from@example.com\r\n", recipients=["to@example.com"], body="testing")
...
def test_plain_message(mailer):
    ...
    msg = mailer.mail(sender="from@example.com", subject="subject", recipients=["to@example.com"], body=plain_text)
    assert plain_text == msg.body
    assert "Content-Type: text/plain" in str(msg)


def test_plain_message_with_attachments(mailer):
    msg = mailer.mail(sender="from@example.com", subject="subject", recipients=["to@example.com"], body="hello")
    msg.attach(data=b"this is a test", content_type="text/plain")
    assert "Content-Type: multipart/mixed" in str(msg)


def test_plain_message_with_unicode_attachments(mailer):
    msg = mailer.mail(subject="subject", recipients=["to@example.com"], body="hello")
...
def test_html_message(mailer):
    ...
    msg = mailer.mail(sender="from@example.com", subject="subject", recipients=["to@example.com"], html=html_text)
...
def test_json_message(mailer):
    ...
    msg = mailer.mail(
        sender="from@example.com", subject="subject", recipients=["to@example.com"], alts={"json": json_text}
    )
...
def test_html_message_with_attachments(mailer):
    ...
    msg = mailer.mail(
        sender="from@example.com", subject="subject", recipients=["to@example.com"], body=plain_text, html=html_text
    )
    msg.attach(data=b"this is a test", content_type="text/plain")
    assert html_text == msg.html
    assert "Content-Type: multipart/alternative" in str(msg)
...
def test_date(mailer):
    before = time.time()
    msg = mailer.mail(
        sender="from@example.com", subject="subject", recipients=["to@example.com"], body="hello", date=time.time()
    )
    after = time.time()
...
def test_send(mailer):
    with mailer.store_mails() as outbox:
        msg = mailer.mail(subject="testing", recipients=["tester@example.com"], body="test")
        msg.send()
...
def test_send_mail(mailer):
    with mailer.store_mails() as outbox:
        mailer.send_mail(subject="testing", recipients=["tester@example.com"], body="test")
        assert len(outbox) == 1
        sent_msg = outbox[0]
